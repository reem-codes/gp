...
\begin{document}
	...
	\mychapter{System Analysis}
		\section{Requirement Specification}
			\paragraph{} These requirements were selected after discussing the proposed idea thoroughly. Top-down approach was used, in which we discussed the general goal of this project, then broke it down even further to smaller manageable units. 
			\subsection{Overview}
	...
	\mychapter{Implementation}
		...
		\paragraph{}As mentioned before, the system has three major components: \boldcolor{Web application}, \boldcolor{mobile application} and \boldcolor{raspberry pi script}. The software requirement was detailed in section \ref{sec:software_req} while the hardware requirements was in section \ref{sec:hardware_req}.  However, after writing the code for each component, an installation has to be made. 
		\subsection{Software Requirements}
		\paragraph{} Jet brain's IDE were used to write the code, specifically \textbf{PyCharm}. For the android application, the official \textbf{Android Studio} was used. \textbf{Postman} was used to test the API.
		
		\subsection{Hardware Requirements}
		\label{hardware_deployment}
		...
		\paragraph{} For the android application, an android mobile is required. Our application is available in google play. \url{https://play.google.com/store/apps/details?id=com.reem_codes.gp_android}
	...
	\mychapter{Testing}
		...
		\paragraph{}In this stage, the concern is in eliminating the majority of bugs that could be easily found with a bit of usage and focused code reading. Basically, addressing those bugs before launching the website and publishing the application. \textit{Unit Testing} and \textit{System and Integration Testing} were used.
			...
			\paragraph{}Here, the concern is on the harmony between the 3 subsystems and their communication. After testing each component under each sub-system individually, a complete system test must be performed to check for any communication error. \textbf{Postman} was used to ensure correct REST API behavior as shown in the \textit{figure \ref{fig:postman}}
			\begin{figure}[H]
				\centering
				\includegraphics[width=\linewidth]{img/postman.png}
				\caption{Postman testing the API}
				\label{fig:postman}
			\end{figure}
		...
		\end{table}
		\section{Test Result}
		\paragraph{}Testing is done for functional and non-functional requirements. If those results are satisfactory, then is is a success. the following tables demonstrate the results.
		...
		\newpage
		\mychapter{Deployment}
			\paragraph{}After implementing the system and testing it, the system was deployed. Again, for each sub-system, the deployment method is different. As mentioned briefly in \textit{Section \ref{hardware_deployment}}. 
			\section{Web Application}
			\paragraph{}First, a domain name and a web server were bought. The official tutorials and help-guides were followed for each service used. Although the web application is a REST API, we made an interface for people who visit it. \textit{Figure \ref{fig:landpage}} shows the interface. The interface can be accessed here \url{gp.reem-codes.com}. For security purposes, SSL certificate was installed from \textbf{Let's Encrypt}. The domain was brought from \textbf{namescheap} and the server from \textbf{DigitalOcean}. 
			\subsection{Raspberry Pi Script}
			\paragraph{} The other use of this page is downloading the raspberry pi script. User can access the website and find the application to be installed in their raspberry pi.
						
			\begin{figure}[H]
				\centering
				\begin{subfigure}[b]{.5\linewidth}
					\includegraphics[width=\linewidth]{img/landpage1.png}
					\caption{Landing Page - Methodology}
				\end{subfigure}
				\begin{subfigure}[b]{.5\linewidth}
					\includegraphics[width=\linewidth]{img/landpage2.png}
					\caption{Landing Page - Gallery}
				\end{subfigure}
				\begin{subfigure}[b]{.5\linewidth}
					\includegraphics[width=\linewidth]{img/landpage3.png}
					\caption{Landing Page - Deliverable}
				\end{subfigure}	
				\caption{Landing Page}
				\label{fig:landpage}	
			\end{figure}		
			\section{Android Application}
			\paragraph{}Android studio offers a way to build the application into \textbf{apk} files then sharing them freely. However, the majority of phones disable downloading from them by default. Unless the user has a good knowledge of how to change the option, they won't be able to use the application. Thus, deploying it in \textbf{google play} was essential.
			\paragraph{} Again, official tutorials were followed. The problems all came from waiting for the approval. Once it was approved, it was published in google play here \url{https://play.google.com/store/apps/details?id=com.reem_codes.gp_android}. \textit{Figure \ref{fig:googleplay} shows the application in google play}.
			\begin{figure}[H]
				\centering
				\includegraphics[width=\linewidth]{img/google_play.png}
				\caption{The Application in Google Play}
	...
	\mychapter{Conclusion}
		...
		\begin{enumerate}
			...
			\item Many problems occurred while deploying the system. Google took 7 days to verify us to allow us to post in google play which took another week. Also, deploying the API took a lot of time to configure and fix the problems. The difficulty comes from the fast that no error messages are displayed. So we had to manage it ourselves.
	...
	%\renewcommand{thesection}{\Alph{section}}
		...
		\includepdf[pages=1-10, width=\linewidth, pagecommand={\pagestyle{fancy}}]{file/api-documentation.pdf}
		
		\newpage\chapter{Code}
		\section{Web Application}
		\lstinputlisting[language=Python, caption={\_\_init\_\_.py}]{code/__init__.py}
		\lstinputlisting[language=Python, caption={view.py}]{code/view.py}
		...
		\section{Android Application}
		\lstinputlisting[language=Java, caption={Command.java}]{code/Command.java}
		\lstinputlisting[language=Java, caption={CommandAdapter.java}]{code/CommandAdapter.java}		
		\lstinputlisting[language=Java, caption={CommandListActivity.java}]{code/CommandListActivity.java}		
		\lstinputlisting[language=Java, caption={NewCommandActivity.java}]{code/NewCommandActivity.java}
		\lstinputlisting[language=Java, caption={ScheduleActivity.java}]{code/ScheduleActivity.java}
		\lstinputlisting[language=Java, caption={activity\_command\_list.xml}]{code/activity_command_list.xml}
		\lstinputlisting[language=Java, caption={command\_item\_adapter.xml}]{code/command_item_adapter.xml}	
