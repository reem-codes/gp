...
\begin{document}
	...
	\begin{center}
		\hspace{0pt}\\
		\vspace{4cm}
		{\scalebox{1}{\large\bfseries\uppercase{Online Home Automation Control System}}}\\			
			\vspace{2cm}
			By: \\
			\begin{table}[H]
				\begin{center}
			...
			\newpage\subsection{Structured Diagrams}
				...
				\boldcolor{executes commands}
					...
					\begin{enumerate}[label*=\arabic*.]
						...
						\item raspberry pi calls the function \textit{EcecuteCommand()} and pass it the command as a parameter.
					...
					\newline\boldcolor{submit a command response to server.}
					...
					\begin{enumerate}[label*=\arabic*.]
						...
						\item  raspberry pi requests the webserver endpoint \textit{/response} with the method \textit{POST}, needed parameter are added to the request body.
	...
	%\renewcommand{thesection}{\Alph{section}}
		...
		\subsection{Similar systems hub design}
		\begin{figure}[H]
